...
class DataManager(ABC):
    ...
    @abstractmethod
    def get_blob(
        self, dataset: Optional[str], bucket_type: str, blob_path: str
    ) -> Optional[bytes]:
        """Cloud-specific blob read. Returns None if blob doesn't exist."""

    @abstractmethod
    def set_blob(
        self, dataset: str, bucket_type: str, blob_path: str, contents: bytes
    ) -> None:
...
class DataManagerGCP(DataManager):
    ...
    def get_blob(
        ...
        bucket_name = (
            f'cpg-{dataset}-{bucket_type}' if dataset else f'cpg-{bucket_type}'
        )
    ...
    def set_blob(
        self, dataset: Optional[str], bucket_type: str, blob_path: str, contents: bytes
    ) -> None:
...
class DataManagerAzure(DataManager):
    ...
    def get_dataset_bucket_url(self, dataset: str, bucket_type: str) -> str:
        ...
        return (
            f'hail-az://{self.get_storage_account(dataset)}/cpg-{dataset}-{bucket_type}'
        )
    ...
    def get_blob(
        ...
        storage_url = (
            'https://' + self.get_storage_account(dataset) + '.blob.core.windows.net'
        )
        container_name = (
            f'cpg-{dataset}-{bucket_type}' if dataset else f'cpg-{bucket_type}'
        )
...
class PathScheme(ABC):
    ...
    def parse(val: str) -> 'PathScheme':
        ...
        if val == 'gs':
            return GSPathScheme()
        if val == 'hail-az':
            return HailAzurePathScheme()
        if val == 'abfs':
            return AzurePathScheme()
        raise ValueError(
            f'Unsupported path format: {val}. Available: gs, hail-az, abfs'
        )
...
class HailAzurePathScheme(PathScheme):
    """
...
class AzurePathScheme(PathScheme):
    """
    Azure Blob Storage path scheme, following the official ABFSS format:
    `abfs://{container}@{account}.dfs.core.windows.net/{path}`
    """

    data_manager = DataManagerAzure()

    def __init__(self):
        config = get_config()
        self.scheme = 'abfs'
        self.container = config['workflow'].get('azure_container', 'cpg')

    def path_prefix(self, dataset: str, category: str) -> str:
        """Build path prefix used in dataset_path"""
        account = AzurePathScheme.data_manager.get_storage_account(dataset)
        return f'{self.container}@{account}.dfs.core.windows.net/{dataset}-{category}'
